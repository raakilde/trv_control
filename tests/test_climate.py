...
async def test_external_temperature_sync(climate_entity):
    """Test external temperature is sent to TRVs via MQTT."""
    climate_entity.hass.services.async_call = AsyncMock()
    climate_entity._attr_current_temperature = 22.5
    
    await climate_entity._async_send_room_temperature_to_trv()
    
    # Should publish MQTT messages for both TRVs
    calls = climate_entity.hass.services.async_call.call_args_list
    mqtt_calls = [c for c in calls if c[0][0] == "mqtt" and c[0][1] == "publish"]
    
    # Each TRV gets 2 MQTT messages (sensor mode + temp)
    assert len(mqtt_calls) == 4
    
    # Check sensor mode messages
    sensor_mode_calls = [c for c in mqtt_calls if "temperature_sensor_select" in c[1]["topic"]]
    assert len(sensor_mode_calls) == 2
    assert all(c[1]["payload"] == "external" for c in sensor_mode_calls)
    
    # Check temperature messages
    temp_calls = [c for c in mqtt_calls if "external_temperature_input" in c[1]["topic"]]
    assert len(temp_calls) == 2
    assert all(c[1]["payload"] == 22.5 for c in temp_calls)


async def test_valve_position_via_mqtt(climate_entity):
    """Test valve position is set via MQTT when entity unavailable."""
    climate_entity.hass.services.async_call = AsyncMock()
    climate_entity._trv_states[MOCK_TRV_1]["valve_entity"] = None
    
    await climate_entity._async_set_valve_position(MOCK_TRV_1, 75)
    
    # Should use MQTT instead of entity
    calls = climate_entity.hass.services.async_call.call_args_list
    mqtt_calls = [c for c in calls if c[0][0] == "mqtt" and c[0][1] == "publish"]
    
    assert len(mqtt_calls) >= 1
    valve_call = mqtt_calls[0]
    assert "valve_opening_degree" in valve_call[1]["topic"]
    assert valve_call[1]["payload"] == 75
...
async def test_trv_status_with_reason(climate_entity):
    ...
    climate_entity._attr_current_temperature = 22.0
    climate_entity._attr_target_temperature = 23.0
    climate_entity._trv_states[MOCK_TRV_1]["return_temp"] = 28.0
    climate_entity._trv_states[MOCK_TRV_1]["valve_position"] = 100
    climate_entity._window_open = False
    climate_entity._attr_hvac_mode = HVACMode.HEAT
    
    status, reason = climate_entity._determine_trv_status_with_reason(MOCK_TRV_1)
    
    assert status == "open"
    assert "room" in reason.lower() and "target" in reason.lower()
